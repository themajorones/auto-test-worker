...
import dev.themajorones.atw.service.handler.AndroidAutonomousTestTaskHandler;
...
public class AndroidTestTaskListener {

    private static final Logger LOGGER = LoggerFactory.getLogger(AndroidTestTaskListener.class);

    private final AndroidAutonomousTestTaskHandler taskHandler;
    private final TaskMessageAckService taskMessageAckService;
    private final ObjectMapper objectMapper = new ObjectMapper();

    @RabbitListener(queues = RabbitMqConstant.Queue.AndroidTest.NAME, ackMode = "MANUAL")
    public void listen(
        String message,
        Channel channel,
        ...
        try {
            ...
            LOGGER.error("Failed to process Android autonomous test command: {}", message, ex);
